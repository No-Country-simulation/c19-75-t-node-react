...
const createUser = async (req, res) => {
    ...
    try {
        ...
        if (esprofesional) {
            const profesionalResult = await transaction.request()
            ...
            if (categorias && categorias.length > 0) {
                for (const categoria of categorias) {
                    await transaction.request()
                        ...
                        .query('INSERT INTO profesionalCategorias (profesionalID, categoriaID) VALUES (@profesional_id, @categoria_id)');
                }
            }
        }
        ...
        res.status(201).json({ message: 'Usuario creado exitosamente', id: usuarioId });
    } catch (err) {
