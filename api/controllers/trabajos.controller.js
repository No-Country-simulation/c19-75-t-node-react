...
const getTrabajosFinalizadosByUserId = async (req, res) => {
  ...
  try {
    ...
    if (userType === 'cliente') {
      //es decir estoy viendo el perfil de un cliente
      // Caso: profesional viendo perfil de un cliente
      trabajosResultado = await pool.request().input('userId', sql.Int, userId) // Pasar el ID del cliente como parámetro
        .query(`
                    SELECT 
                        t.titulo AS titulo, 
                        t.fotos AS fotos, 
      ...
      //es decir, estyo viendo le perfil de un profesional
      // Caso: cliente viendo perfil de un profesional
      trabajosResultado = await pool.request().input('userId', sql.Int, userId) // Pasar el ID del profesional como parámetro
        .query(`
                    SELECT 
                        t.titulo AS titulo, 
                        t.fotos AS fotos, 
