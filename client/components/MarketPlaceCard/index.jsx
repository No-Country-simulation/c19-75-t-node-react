...
const MarketPlaceCard = ({ keyProp, data, pathname, categoria }) => {
          ...
          {item.cant_works_done > 0 ? (
            <>
              ...
              {Array.from({ length: 5 }).map((_, idx) => (
                <Star key={idx} otherStyles={`${styles.icon} ${idx < item.rating ? styles.fill : ''}`} />
              ))}
  ...
  const handleClick = () => {
    console.log(itemId);
    if (isWorker) {
      router.push(`/usuario/${itemId}`);
    } else {
      router.push(`/laburos/${CATEGORY_MAP[item?.category]}/${itemId}`);
    }
  };
  ...
  return (
    <div className={styles.card} key={keyProp + item?.title}>
      ...
      <div className={`${styles.info} ${isWorker ? styles.info__grid_workers : styles.info__grid_laburos}`}>
