...
const Header = () => {
  ...
  const handleCreateJob = () => {
    router.push('/usuario/crear-trabajo');
  };
    ...
    return (
      <>
        {CATEGORIES.map((category) => (
          <Link key={category.url} href={`/${redirectPage}/${category.url}`} className={styles.link}>
            {category.name}
          </Link>
        ))}
      </>
          ...
          {!userSessionData ? (
            ...
            <>
              {!userSessionData?.isWorker && (
                <button className={styles.buttonCliente} onClick={handleCreateJob}>
